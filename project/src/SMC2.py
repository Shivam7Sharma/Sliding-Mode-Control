...
class Quadrotor():
	...
	#trajectories
		...
		Fx=self.m*(-Kp*(xyz[0]-pos[0])-Kd*(xyz_dot[0]-vel[0])+acc[0])
		Fy=self.m*(-Kp*(xyz[1]-pos[1])-Kd*(xyz_dot[1]-vel[1])+acc[1])
		theta_d=asin(Fx/u1)
		phi_d=asin(-Fy/u1)
		...
		sat_phi=min(max(s_phi/bdry_phi, -1), 1)
		...
		sat_theta=min(max(s_theta/bdry_theta, -1), 1)
		...
		sat_psi=min(max(s_psi/bdry_psi, -1), 1)
		...
		motor_vel=np.sqrt(np.matmul(amat,u))
		motor_vel = np.clip(motor_vel, a_min=0, a_max=2618.0)
	...
	def save_data(self):
		...
		with open("/home/neet/rbe502_project/src/project/scripts/log.pkl","wb") as fp:
